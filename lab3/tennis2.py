class TennisGame2:
    ...
    def score(self):
        ...
        if self.p1points == self.p2points:
            return f"{score_names[self.p1points]}-All" if self.p1points < 3 else "Deuce"
            
        #ktoś ma >= 4 punktów
        if max(self.p1points, self.p2points) >= 4:
            leader = self.player1_name if self.p1points > self.p2points else self.player2_name
            difference = abs(self.p1points - self.p2points)
            return f"Win for {leader}" if difference >= 2 else f"Advantage {leader}"   
